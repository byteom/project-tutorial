...
const parseTutorialText = (text: string, prompt: string): Project => {
  ...
  while(descriptionEndIndex < lines.length && !lines[descriptionEndIndex].startsWith('## ')){
    descriptionEndIndex++;
  }
  ...
  for (let i = descriptionEndIndex; i < lines.length; i++) {
    const line = lines[i];
    if (line.startsWith('## ')) {
      if (currentStepTitle) {
        steps.push({
      ...
      currentStepTitle = line.replace('## ', '').replace(/Step \d+:\s*/, '').trim();
      currentStepContent = [];
    } else {
